...
# (susScr3, SGSC Sscrofa10.2 (NCBI project 13421, GCA_000003025.4, WGS AEMK01)).
# http://hgdownload.soe.ucsc.edu/goldenPath/susScr3/bigZips/

# Create and enter working directory:
mkdir -p /home/workspace/ccorreia/globin/UCSC/transcriptomes/pig/source_file
cd !$

# Download and unzip the transcriptome:
nohup wget http://hgdownload.soe.ucsc.edu/goldenPath/susScr3/bigZips/refMrna.fa.gz &
gunzip refMrna.fa.gz
mv refMrna.fa ssc_refMrna.fa
...
cd /home/workspace/ccorreia/globin/UCSC/transcriptomes/pig

# Build an index for quasi-mapping:
nohup salmon index -t \
/home/workspace/ccorreia/globin/UCSC/transcriptomes/pig/source_file/ssc_refMrna.fa \
...
# Create and enter working directory:
mkdir -p /home/workspace/ccorreia/globin/UCSC/salmon_quant/pig
cd !$

# Quantify transcripts from one FASTQ file to check if it works well:
salmon quant -i /home/workspace/ccorreia/globin/UCSC/transcriptomes/pig/pig_index \
-l A --seqBias --gcBias \
...
echo "salmon quant -i /home/workspace/ccorreia/globin/UCSC/transcriptomes/pig/pig_index \
...
for file in `find /home/workspace/ccorreia/globin/UCSC/salmon_quant/pig \
-name quant.sf`; \
...
mkdir /home/workspace/ccorreia/globin/UCSC/salmon_quant/pig/pig_TPM
for file in `find /home/workspace/ccorreia/globin/UCSC/salmon_quant/pig \
-name 7*quant.sf`; \
do cp $file -t /home/workspace/ccorreia/globin/UCSC/salmon_quant/pig/pig_TPM; \
done

# Transfer all files from Rodeo to laptop:
scp -r ccorreia@remoteserver:/home/workspace/ccorreia/globin/UCSC/salmon_quant/pig/pig_TPM .
...
UCSC_summary_pig.txt" >> UCSC_summary_pig.sh
done

chmod 755 UCSC_summary_pig.sh
./UCSC_summary_pig.sh

sed -i $'1 i\\\nFile_name Library_type Total_fragments Total_reads Mapping_rate(%)' \
UCSC_summary_pig.txt

# Transfer summary file from Rodeo to laptop:
scp -r ccorreia@remoteserver:/home/workspace/ccorreia/globin/UCSC/salmon_quant/pig/UCSC_summary_pig.txt .
